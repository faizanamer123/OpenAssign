    ...
    <div className="min-h-screen bg-[#fcfbf8]">
      ...
      <div className="px-4 py-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-[#1c180d] mb-2">Browse Assignments</h1>
            <p className="text-[#9e8747]">
              Find assignments to solve and earn points. Help your fellow students while building your reputation.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-[#9e8747]">No assignments found matching your criteria.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredAssignments.map((assignment) => (
                <Card key={assignment.id} className="border-[#e9e2ce] bg-[#fcfbf8] hover:shadow-lg transition-shadow">
                  <CardHeader>
                    ...
                    <CardTitle className="text-[#1c180d] text-lg line-clamp-2">{assignment.title || "No Title"}</CardTitle>
                    <CardDescription className="text-[#9e8747] line-clamp-3">{assignment.description || "No Description"}</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center gap-2 text-sm text-[#9e8747]">
                        ...
                        <span>Posted by {assignment.createdByUsername || "Anonymous"}</span>
                      </div>
                      <Link href={`/assignment/${assignment.id}`}>
                        <Button className="w-full bg-[#fac638] text-[#1c180d] hover:bg-[#fac638]/90">
                          {assignment.status === "solved" ? "View Solution" : "View & Solve"}
                        </Button>
                      </Link>
                    </div>
                  </CardContent>
